...
class DataLoader(object):
    ...
    def batches(self, setpath="datasets/Youku/Youku/train/HR", batch_size=16, complete_batch_only=False):
...
class DataLoader(object):
    ...
    # 关键是这个函数的改造,可以根据自己数据集的格式修改
    def pair(self, fp):
        hr = self.imread(fp)
        lr = self.imread(fp.replace('HR','LR'))
        lr, hr = self.rotate(lr, hr)
        lr, hr = self.flip(lr, hr)
        lr, hr = self.crop(lr, hr)
        lr = np.asarray(lr)
